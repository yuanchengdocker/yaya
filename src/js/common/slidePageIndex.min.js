(function($) {
    ...
    window.slidePageIndex = {
        ...
        'index': function(index){
            if(index>0&&index!=keyIndex+1){
                ...
                var endheight = $(".index-item").eq(index).children().height();
    ...
    function initDom(opt) {
        ...
        slidePageIndex.index(opt.index)// 初始化指定页码
    ...
    function slideScroll(target){
        ...
        if((itemheight-windowH)>20){
            $(opt.pageContainer).eq(target-1).on('scroll',function(e){
                ...
                slidePageIndex.canSlide = isBottom || isTop;
                slidePageIndex.canPrev = isTop && !isBottom;
                slidePageIndex.canNext = isBottom && !isTop;
                slidePageIndex.isScroll = !(slidePageIndex.canSlide)
            });
        }else{
            slidePageIndex.canPrev = true;
            slidePageIndex.canNext = true;
            slidePageIndex.canSlide = true;
            slidePageIndex.isScroll = false;
        }
    ...
    function initEvent(opt) {
        ...
        function wheelFunc(e){
            ...
            if(e.wheelDeltaY<0||e.wheelDelta<0||e.detail>0){
                slidePageIndex.canNext&&delay&&slidePageIndex.next();
            }else if(e.wheelDeltaY>0||e.wheelDelta>0||e.detail<0){
                slidePageIndex.canPrev&&delay&&slidePageIndex.prev();
            }
        ...
        if(!!opt.useKeyboard){
            document.onkeydown = function(e){
                if(e.keyCode=='40'&&delay&&keyIndex<pageCount-1){
                    slidePageIndex.canNext&&slidePageIndex.next();
                }else if(e.keyCode == '38'&&delay&&keyIndex>0){
                    slidePageIndex.canPrev&&slidePageIndex.prev();
                }
            }
        }
        //-- 获取触控开始位置
        ...
        document.getElementById('slidePage-container-index').addEventListener('touchstart',function(e){
            touchY = e.touches[0].clientY
        })
        //-- 判断触控移动方向
        document.getElementById('slidePage-container-index').addEventListener('touchmove',function(e){
            var offsetY = e.touches[0].clientY-touchY
            !slidePageIndex.canPrev&&offsetY>5&&(slidePageIndex.isScroll=true)    //-- 滚动到底部往上滑可继续滚动条滚动
            !slidePageIndex.canNext&&offsetY<-5&&(slidePageIndex.isScroll=true)   //-- 滚动到顶部往下滑可继续滚动条滚动
            !slidePageIndex.isScroll&&e.preventDefault()
        });
        //-- 手势滑动
